...
/// indices (see grid.rs) a ball may be sampled from. A fixed cell is just
...
/// deliberate authoring choice that may include a hard cut. Falls back to
/// the last-sampled candidate if nothing qualifies in time.
pub fn sample_exercise(exercise: &Exercise, grid: &GridSpec, pockets: &[Pocket]) -> (Vector3, Vector3) {
    let mut candidate = (Vector3::zero(), Vector3::zero());
...
mod tests {
    ...
    fn sampled_positions_land_within_the_requested_range_and_clear() {
        ...
        let exercise = Exercise {
            label: None,
            object_ball: CellRange { row: [grid.row_center, grid.row_center], col: [grid.col_center, grid.col_center] },
        ...
        for _ in 0..50 {
            ...
            let (expected_x, expected_z) = grid.world_pos(grid.row_center, grid.col_center);
    ...
    /// too tight to ever clear separation) that a hand-rolled test fixture
    /// wouldn't.
    #[test]
