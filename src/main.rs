...
mod shaders;
mod touch_ui;
use touch_ui::{menu_ui, puzzle_menu_ui, touch_ui};
mod camera;
...
fn main() {
    ...
    while !rl.window_should_close() && !state.quit_requested {
        ...
        let ui = touch_ui(screen_w, screen_h, reset_label, state.ghost_aim_visible, state.active_set.is_some());
        let menu = menu_ui(screen_w, screen_h);
        let set_names: Vec<String> = assets.puzzle_sets.iter().map(|s| s.name.clone()).collect();
        let puzzle_menu = puzzle_menu_ui(screen_w, screen_h, &set_names);
        ...
        // while the help popup or any menu is open (the frozen 3D scene
